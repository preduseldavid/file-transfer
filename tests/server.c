...
int main(void)
{
	...
	if (rc == -1) {
		fprintf(stdout, "unable to bind to socket: %s\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	while (1) {
		...
		if (strcmp(filename, "quit") == 0) {
			fprintf(stdout, "quit command received, shutting down server\n");
			break;
		}
		
		fprintf(stdout, "received request to send file %s\n", filename);
		...
		if (rc == -1) {
			fprintf(stdout, "Failed to send the file size\n");
			exit(EXIT_FAILURE);
		}
		
		fprintf(stdout, "File size to be sent [%"PRIu64"]\n", stat_buf.st_size);
		...
		while (sent < stat_buf.st_size && (rc = sendfile (desc, fd, &offset, stat_buf.st_size))) {
			if (rc == -1) {
				fprintf(stdout, "error from sendfile: %s\n", strerror(errno));
				exit(EXIT_FAILURE);
			}
			else sent += rc;
		}
		...
		if (sent != stat_buf.st_size) {
			fprintf(stdout, "incomplete transfer from sendfile: %d of %d bytes\n",
					rc,
